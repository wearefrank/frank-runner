...
if [[ `pwd` == ${BASH_SOURCE%/*}* ]] then
  # Workaround for VSCode Task Explorer on Mac
  FR_DIR="${BASH_SOURCE%/*}"
fi
...
"${FR_DIR}build/apache-ant-1.10.10/bin/ant" -Dfr.jdk.8.dir="${JDK_8_DIR}" -Dfr.jdk.11.dir="${JDK_11_DIR}" -Dfr.jdk.17.dir="${JDK_17_DIR}" -emacs -buildfile "${FR_DIR}build.xml" "$@" restart