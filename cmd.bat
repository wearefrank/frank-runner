:: This file is generated by build.xml
@echo off
setlocal enabledelayedexpansion
if not exist "%~dp0download\" (
	mkdir "%~dp0download"
)
if not exist "%~dp0build\tmp\build\" (
	mkdir "%~dp0build\tmp\build"
)
set DOWNLOAD_HELP=download https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.6+7/OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip manually, move it to %~dp0download and restart this script
if not exist "%~dp0download\OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip" (
	echo "In case of errors %DOWNLOAD_HELP%"
	curl -f -o "%~dp0download\OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip.tmp" -L https://github.com/adoptium/temurin21-binaries/releases/download/jdk-21.0.6+7/OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip
	if !errorlevel! neq 0 (
		echo "Please %DOWNLOAD_HELP%"
		goto error
	)
	move "%~dp0download\OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip.tmp" "%~dp0download\OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip"
)
if not exist "%~dp0build\jdk-21.0.6+7\" (
	tar --exclude=*/demo --exclude=*/sample --exclude=*/manual --exclude=*/src.zip -xvf "%~dp0download\OpenJDK21U-jdk_x64_windows_hotspot_21.0.6_7.zip" -C "%~dp0build\tmp\build"
	if !errorlevel! neq 0 (
		echo "Please %DOWNLOAD_HELP%"
		goto error
	)
	move "%~dp0build\tmp\build\jdk-21.0.6+7" "%~dp0build\jdk-21.0.6+7"
	RUN_INSTALL="true"
)
set DOWNLOAD_HELP=download https://archive.apache.org/dist/ant/binaries/apache-ant-1.10.15-bin.zip manually, move it to %~dp0download and restart this script
if not exist "%~dp0download\apache-ant-1.10.15-bin.zip" (
	echo "In case of errors %DOWNLOAD_HELP%"
	curl -f -o "%~dp0download\apache-ant-1.10.15-bin.zip.tmp" -L https://archive.apache.org/dist/ant/binaries/apache-ant-1.10.15-bin.zip
	if !errorlevel! neq 0 (
		echo "Please %DOWNLOAD_HELP%"
		goto error
	)
	move "%~dp0download\apache-ant-1.10.15-bin.zip.tmp" "%~dp0download\apache-ant-1.10.15-bin.zip"
)
if not exist "%~dp0build\apache-ant-1.10.15\" (
	tar --exclude=*/demo --exclude=*/sample --exclude=*/manual --exclude=*/src.zip -xvf "%~dp0download\apache-ant-1.10.15-bin.zip" -C "%~dp0build\tmp\build"
	if !errorlevel! neq 0 (
		echo "Please %DOWNLOAD_HELP%"
		goto error
	)
	move "%~dp0build\tmp\build\apache-ant-1.10.15" "%~dp0build\apache-ant-1.10.15"
)
set DOWNLOAD_HELP=download https://github.com/mozilla/rhino/releases/download/Rhino1_7_15_Release/rhino-1.7.15.zip manually, move it to %~dp0download and restart this script
if not exist "%~dp0download\rhino-1.7.15.zip" (
	echo "In case of errors %DOWNLOAD_HELP%"
	curl -f -o "%~dp0download\rhino-1.7.15.zip.tmp" -L https://github.com/mozilla/rhino/releases/download/Rhino1_7_15_Release/rhino-1.7.15.zip
	if !errorlevel! neq 0 (
		echo "Please %DOWNLOAD_HELP%"
		goto error
	)
	move "%~dp0download\rhino-1.7.15.zip.tmp" "%~dp0download\rhino-1.7.15.zip"
)
if not exist "%~dp0build\rhino1.7.15\" (
	tar --exclude=*/demo --exclude=*/sample --exclude=*/manual --exclude=*/src.zip -xvf "%~dp0download\rhino-1.7.15.zip" -C "%~dp0build\tmp\build"
	if !errorlevel! neq 0 (
		echo "Please %DOWNLOAD_HELP%"
		goto error
	)
	move "%~dp0build\tmp\build\rhino1.7.15" "%~dp0build\rhino1.7.15"
)
if not exist "%~dp0build\apache-ant-1.10.15\lib\rhino-1.7.15.jar" (
	del "%~dp0build\apache-ant-1.10.15\lib\rhino-*.jar"
	copy "%~dp0build\rhino1.7.15\lib\rhino-*.jar" "%~dp0build\apache-ant-1.10.15\lib\"
)
set JDK_8_DIR=%~dp0%build\jdk8u442-b06
set JDK_11_DIR=%~dp0%build\jdk-11.0.26+4
set JDK_17_DIR=%~dp0%build\jdk-17.0.14+7
set JDK_21_DIR=%~dp0%build\jdk-21.0.6+7
if not exist "%JDK_8_DIR%" (
	set RUN_INSTALL=true
)
if not exist "%JDK_11_DIR%" (
	set RUN_INSTALL=true
)
if not exist "%JDK_17_DIR%" (
	set RUN_INSTALL=true
)
if not exist "%~dp0build\apache-maven-3.9.9" (
	set RUN_INSTALL=true
)
set JAVA_HOME=%JDK_21_DIR%
set ANT_HOME=%~dp0build\apache-ant-1.10.15
if "%RUN_INSTALL%" == "true" (
	call "%~dp0build\apache-ant-1.10.15\bin\ant" -emacs -buildfile "%~dp0build.xml" install
)
set PATH=%JAVA_HOME%\bin;%ANT_HOME%\bin;%~dp0build\apache-maven-3.9.9\bin;%PATH%
C:\Windows\System32\cmd.exe /k "echo JAVA : %JAVA_HOME%& echo ANT  : %ANT_HOME%& echo MAVEN: %~dp0build\apache-maven-3.9.9"